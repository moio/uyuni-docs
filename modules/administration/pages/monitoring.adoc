...
{productname} Server and Proxy are able to provide self-health metrics, and you can also install and manage a number of Prometheus exporters on Salt clients.
...
[NOTE]
====
Prometheus fetches metrics using a pull mechanism, so the server must be able to establish TCP connections to monitored clients. Clients must have the corresponding open ports and be reachable over the network.
====
...
.Procedure: Installing Prometheus via {productname} {webui}
...
. In the ``{productname} Server`` section, enter valid {productname} API credentials. Make sure that the credentials you have entered allow access to the set of systems you want to monitor.
...
. Open the configuration file at [path]``/etc/prometheus/prometheus.yml`` and add the configuration block below. Make sure to replace `server.url` with your {productname} server URL and adjust `username` and `password` fields to match valid {productname} credentials.
+
----
...
[NOTE]
{productname} provides pre-build dashboards for Server self-health, basic client monitoring, and more. In the formula configuration page, you can select which dashboards to provision in your Grafana setup.
...
With {productname}{nbsp}4, you can enable the Server to expose Prometheus self-health metrics. Those metrics are made available by instrumentation of the Java application combined with Prometheus exporters. Together, they provide metrics about the underlying Hardware, Operating System and {productname} internals.
...
Prometheus metrics exporters can be installed and configured on Salt clients via Formulas. The packages are available from the {productname} client tools channels, and can be enabled and configured directly in the {productname} {webui}.

The following exporters can be installed on managed systems:
...
When you have the exporters installed and configured, you can start using Prometheus to scrape metrics from monitored systems. If you have configured your monitoring server via {productname} {webui}, the metrics collection will happen automatically.

.Procedure: Configuring Prometheus Exporters on a System

. In the {productname} {webui}, open the details page of the system to be monitored, and navigate to the menu:Formulas tab.
